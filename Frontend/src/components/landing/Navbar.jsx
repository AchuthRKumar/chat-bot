...
const Navbar = ({ onBookCallClick}) => {
  ...
  const emailToCopy = "achuthrkumar@gmail.com";
        ...
        <ul className="hidden md:flex items-center space-x-11">
          ...
          <li>
            <button
              ...
              className="relative group h-7 w-24 overflow-hidden focus:outline-none py-2"
              style={{ fontFamily: 'inherit', fontSize: 'inherit' }}
            >
              <span
                className={`absolute inset-0 flex items-center justify-center font-semibold transition-opacity duration-300 ${
                  buttonText === 'Copied!' ? 'opacity-100' : 'opacity-0'
                }`}
              >
                Copied!
              </span>
                ...
                className={`absolute inset-0 transition-all duration-300 ease-in-out
                  ${
                    buttonText === 'Copied!'
                      ...
                      : 'group-hover:-translate-y-full opacity-100'
                  }
                `}
              >
